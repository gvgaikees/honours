package tracks.levelGeneration.submission.levelGenerators;
...
/**
 * This level generator makes levels with:
 * - Low Enemy Diversity
 * - High Enemy Density
 * - High Enemy Clustering
 */
public class LevelGenerator0 extends AbstractLevelGenerator{

	/**
	 * Add Borders to the generated level
	 */
	...
	/**
	 * Constructor for the LevelGenerator0 where it initialize the random object used.
	 ...
	 */
	public LevelGenerator0(GameDescription game, ElapsedCpuTimer elapsedTimer){
		random = new Random();
		minSize = 4;
		maxSize = 18;
	...
	/**
	 * Get the harmful characters that are described in the level mapping
	 * @param gameDescription	game description object to get all data
	 * @return					character of the first solid object found or null otherwise
	 */
	private Character getHarmfulCharacter(GameDescription gameDescription, String spriteName){
		ArrayList<String> solidSprites = gameAnalyzer.getHarmfulSprites();
		int minValue = Integer.MAX_VALUE;
		Character result = ' ';
		for(Entry<Character, ArrayList<String>> entry:gameDescription.getLevelMapping().entrySet()){
			if(entry.getValue().contains(spriteName)){
				if(entry.getValue().size() < minValue){
					result = entry.getKey();
				}
			}
	...
	public String generateLevel(GameDescription game, ElapsedCpuTimer elapsedTimer) {
		...
		// Get a modifier for the size of the level
		double size_constant = 2;

		//Get a width and random height value based on the length of game sprites
		//and it should be in between maxSize and minSize
		int width = (int) (Math.max(minSize, sprites.size() * (1 + 0.25 * random.nextDouble())) * size_constant);
		int length = (int) (Math.max(minSize, sprites.size() * (1 + 0.25 * random.nextDouble())) * size_constant);
		...
		for(Entry<Character, ArrayList<String>> pair:game.getLevelMapping().entrySet()){
		...
		gameAnalyzer.getHarmfulSprites().forEach(sprite -> {
			Character c = getHarmfulCharacter(game, sprite);

			// DECREASE ENEMY DIVERSITY (only get half of the harmful sprites)
			if (c % 2 == 0) {
				choices.add(c);
				choices.add(c);
				choices.add(c);
