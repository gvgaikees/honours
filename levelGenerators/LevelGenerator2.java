...
public class LevelGenerator2 extends AbstractLevelGenerator{
	 ...
	 * @return					character of the first solid object found or null otherwise
	 */
	private Character getSpecificSolidCharacter(GameDescription gameDescription, String spriteName){
		ArrayList<String> solidSprites = gameAnalyzer.getSolidSprites();
		int minValue = Integer.MAX_VALUE;
		Character result = ' ';
		for(Entry<Character, ArrayList<String>> entry:gameDescription.getLevelMapping().entrySet()){
	...
	public String generateLevel(GameDescription game, ElapsedCpuTimer elapsedTimer) {
		...
		gameAnalyzer.getHarmfulSprites().forEach(sprite -> {
			Character c = getSpecificSolidCharacter(game, sprite);
			choices.add(c);
			choices.add(c);
			choices.add(c);
		});
